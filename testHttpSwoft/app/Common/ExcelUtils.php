...
class ExcelUtils
{
    ...
    {
        try {
            Log::debug(__METHOD__.' $file:'.$file);
            ...
            return self::convertExcelCellToData($data);
    ...
    public static  function exportExcel(array $datas, string $fileName = '', array $options = []): bool
    {
        try {
            ...
            $datas = self::convertDataToExcelCell($datas);
            Log::info(__METHOD__.' $datas:'.json_encode($datas));
            ...
            foreach ($datas as $sKey => $sItem) {
                ...
                Log::info(__METHOD__.' $sKey:'.$sKey.' $sItem:'.json_encode($sItem));
                foreach ($sItem as $itemKey => $itemVal)
                {
                    $activeSheet->setCellValueExplicit($itemKey, $itemVal, $pDataType);
                }
...
//            ob_clean();
     ...
     * @param $data
     */
    public static  function convertDataToExcelCell($data)
    {
        $newRetData = [];
        foreach ($data as $key => $dataItem)
        {
            Log::info(__METHOD__.' key:'.$key.' $dataItem:'.json_encode($dataItem));
            if($key == 0)
            {
                $colNames = array_keys($dataItem);
                array_push($newRetData,self::buildRowData(1,$colNames));
            }
            $colValues = array_values($dataItem);
            array_push($newRetData,self::buildRowData($key + 2,$colValues));
        }
        return $newRetData;
    }
     ...
     */
    public static function buildRowData($rowIndex, $rowData)
    {
        $cellColBeginChar = 'A';
        $retRowData = [];
        foreach ($rowData as $key => $cellItem)
        {
            $colPos = chr(ord($cellColBeginChar) + $key).''.$rowIndex;
            $retRowData[$colPos] = $cellItem;
            Log::info(__METHOD__.' $colPos:'.$colPos.' $cellItem:'.$cellItem);
        }
        Log::info(__METHOD__.' $rowIndex:'.$rowIndex.' $retRowData:'.json_encode($retRowData));
        return $retRowData;
    }
     ...
     * @throws \Swoft\Bean\Exception\ContainerException
     */
    public static function convertExcelCellToData($data)
    {
        $retData =  array_values($data);
        if(count($retData) > 1)
        {
            $newRetData = [];
            $colMap = [];
            $unitObj = [];
            foreach ($retData as $key => $retItem)
            {
                unset($unitObj);
                $unitObj = [];
                Log::info(__METHOD__.' key:'.$key.' $retItem:'.json_encode($retItem));
                if($key == 1)
                {
                    Log::info(__METHOD__.' $colMap:'.json_encode($colMap));
                }
                $singleIndex = 0;
                foreach ($retItem as $singleKey => $singleVal)
                {
                    //  Log::info(__METHOD__.' $singleKey:'.$singleKey.' $singleVal:'.$singleVal.' $singleIndex:'.$singleIndex);
                    if($key == 0)
                    {
                        array_push($colMap,$singleVal);
                    }
                    else{
                        $unitObj[$colMap[$singleIndex]] = $singleVal;
                    }
                    $singleIndex++;
                }
                if($key > 0)
                {
                    Log::info(__METHOD__.' $unitObj:'.json_encode($unitObj));
                    array_push($newRetData,$unitObj);
                }
            }
            return $newRetData;
        }
